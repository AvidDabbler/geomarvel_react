...
export const WebMapView = () => {
  ...
  const loadMap = async () => { 
    ...
    //   dom
    // ]) => {
    const [ArcGISMap, MapView] = await loadModules(['esri/Map', 'esri/views/MapView']);
  ...
  const addLayer = (map, config) => {
    loadModules([
      "esri/layers/GeoJSONLayer",
    ], { css: true })
    .then(([GeoJSONLayer])=>{
      map.add(new GeoJSONLayer(config))
    })
  }
  ...
  useEffect(async()=>{
      ...
      addLayer(map, TreeConfig(treeURL))
